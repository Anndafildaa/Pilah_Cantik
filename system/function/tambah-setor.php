...
if (isset($_POST['simpan'])) {
  ...
  $checkQuery = "SELECT * FROM setor WHERE jenis_sampah = '$jenis_sampah'";
  $result = mysqli_query($conn, $checkQuery);
  
  if (mysqli_num_rows($result) > 0) {
    echo "<script>alert('Jenis sampah sudah ada, harap gunakan jenis sampah yang lain!')</script>";
  } else {
    $query = "INSERT INTO setor(id,tanggal_setor,id_nasabah,jenis_sampah,berat,harga,total,id_admin) 
              VALUES (NULL, '$tanggal_setor', '$id_nasabah', '$jenis_sampah', '$berat', '$harga', '$total', '$id_admin')";
